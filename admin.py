...
class DataPointGeneratorIdentifierFilter(SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        for generator_definition in DataGeneratorDefinition.objects.all().order_by('name'):
            values.append((generator_definition.pk,
                           generator_definition.name,))
...
class DataFileAdmin(admin.OSMGeoAdmin):
    list_display = ('content_file', 'content_type',
                    'identifier', 'data_point', 'data_bundle',)
...
class DataSourceAdmin(admin.OSMGeoAdmin):
    list_display = ('name', 'identifier', 'group',
                    'suppress_alerts', 'performance_metadata_updated',)
...
class DataSourceAlertAdmin(admin.OSMGeoAdmin):
    ...
    list_filter = ('active', 'created', 'updated',
                   'alert_level', 'generator_identifier',)
...
class AppConfigurationAdmin(admin.OSMGeoAdmin):
    list_display = ('name', 'evaluate_order', 'id_pattern',
                    'context_pattern', 'is_valid', 'is_enabled',)
    search_fields = ('name', 'id_pattern', 'context_pattern',
                     'configuration_json',)
...
class DataServerAccessRequestPendingAdmin(admin.OSMGeoAdmin):
    list_display = ('user_identifier', 'request_type',
                    'request_time', 'successful', 'processed',)
...
class DeviceIssueAdmin(admin.OSMGeoAdmin):
    ...
    list_filter = ('state', 'created', 'last_updated', 'stability_related', 'uptime_related', 'responsiveness_related', 'battery_use_related',
                   'power_management_related', 'data_volume_related', 'data_quality_related', 'bandwidth_related', 'storage_related', 'configuration_related', 'location_related',)
